...
const Home = () => {
  ...
  const handleSearch = () => {
    if (searchTerm.trim() !== '') {
      navigate('/events', { state: { keyword: searchTerm.toLowerCase() } });
      setSearchTerm('');
    }
  };
  ...
  const weeklyEvents = [
    { id: 1, title: 'Festival Electro', date: '30 Avril 2025', location: 'Lyon', image: concertImg, lat: 45.764043, lng: 4.835659 },
  ...
  const handleDetailsClick = (event) => setSelectedEvent(event);
  const handleCloseDetails = () => setSelectedEvent(null);
  const handleChooseEvent = (event) => navigate('/events', { state: { selectedEvent: event } });
      ...
      <section className="hero-section">
        ...
        <div className="hero-content">
          ...
          <div className="search-container">
            ...
            />
            <button onClick={handleSearch}>Explorer</button>
          </div>
